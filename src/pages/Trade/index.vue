<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="item in userAddress" :key="item.id" @click="switchAddress(item)">
        ...
        <p>
          ...
          <span class="s3" v-if="item.isDefault == 1 ? true : false">默认地址</span>
        </p>
      </div>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{receivingAddress.fullAddress}}</span>
        收货人：<span>{{receivingAddress.consignee}}</span>
        <span>{{receivingAddress.phoneNum}}</span>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async submitOrder() {
      const { orderInfo, receivingAddress, msg } = this
      const tradeNo = orderInfo.tradeNo

      const data = {
        consignee: receivingAddress.consignee,
        consigneeTel: receivingAddress.phoneNum,
        deliveryAddress: receivingAddress.fullAddress,
        paymentWay: 'ONLINE',
        orderComment: msg,
