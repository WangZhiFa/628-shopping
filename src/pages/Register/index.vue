<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <label>登录密码:</label>
        <input placeholder="请输入你的密码" v-model="password" name="password" v-validate="{ required: true, regex: /^[0-9A-Za-z]{6,12}$/ }" :class="{ invalid: errors.has('password') }" />
...
export default {
  ...
  methods: {
    async getVerificationCode() {
      ...
      try {
        await this.$store.dispatch('user/getVerificationCode', this.phone)
    ...
    async userRegister() {
      try {
        ...
        if (success) {
          const { password, phone, code } = this
          await this.$store.dispatch('user/userRegister', { password, phone, code })
          alert('注册成功')
